...
export function VerifyEmailStep() {
  ...
  }, [dispatch, state.email, router, locale]);
  ...
  return (
    <div className="space-y-6">
      <button type="button" onClick={() => router.push(`/${locale}/onboarding/email`)} className="absolute -top-12 -left-2 text-muted-foreground hover:text-foreground p-2 rounded-md hover:bg-accent hover:text-accent-foreground transition-colors">
        <ArrowLeft className="w-5 h-5" />
      </button>

      <div className="space-y-2 text-center">
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">
          {t("step3Title", { firstName: getFirstName(state.fullName) })}
        </h1>
        <p className="text-sm text-muted-foreground">
          {t("step3Subtitle", { email: state.email })}
        </p>
      </div>
